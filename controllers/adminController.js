...
module.exports = {
    ...
    home: async (req, res) => {
        try {
            ...
            res.render('admin/home', { admin: req.session.admin, users, products, sold, blocked, Brand, Type, Fuel, Banner, Coupen })
        } catch (err) {
            next(err)
        }
    ...
    addproduct: async (req, res) => {
        ...
        // // original image
        // let originalImage = ProductModel.image;

        // // file name for cropped image
        // let outputImage = ProductModel.image
    ...
    allCoupen: async (req, res) => {

        const coupens = await coupenModel.find({}).then((data) => {
            console.log(data)
            let coupens = data
            res.render('admin/viewCoupen', { coupens, index: 1, admin: req.session.admin })
        })

    },

    addCoupenPage: async (req, res) => {

        res.render('admin/addCoupen', { admin: req.session.admin })
    },

    addCoupen: async (req, res) => {

        const { coupenName, discount, maximum } = req.body

        const newCoupen = coupenModel({
            coupenName,
            discount,
            maximum,
        });
        ...
        // .catch((err) => {
        //     console.log(err.message);
        //     res.redirect("/admin/addCoupenPage");
        // });
